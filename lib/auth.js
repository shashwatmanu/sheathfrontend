  ...
  try {
    ...
    try {
      ...
      if (profileResponse.ok) {
        ...
        setCookie('is_admin', profile.is_admin === true ? 'true' : 'false', 7);
        console.log("[auth.js] login() profile and is_admin cookie saved");
      } else {
...
export function isAuthenticated() {
  ...
  // Fix the Infinite Redirect Loop Issue:
  // Next.js middleware relies on the "access_token" cookie to allow access to "/dashboard".
  // If the cookie is missing (e.g. cleared by user or expired), but localStorage still has the token,
  // the client thinks we are authenticated (and redirects from /login to /dashboard),
  // but the server thinks we are NOT authenticated (and redirects from /dashboard to /login).
  // Thus we sync the cookie from localStorage here if it's missing.
  const cookieMatch = document.cookie.match(new RegExp('(^| )access_token=([^;]+)'));
  if (!cookieMatch) {
    console.warn("[auth.js] isAuthenticated(): Cookie missing but token in localStorage. Restoring cookie to prevent redirect loop.");
    setCookie('access_token', token, 7);

    // Also restore username cookie if possible
