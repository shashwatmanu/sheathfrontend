...
import { isAuthenticated, logout, getUsername, authenticatedFetch, isAdmin, getStoredProfile, getUserProfile } from "../../../lib/auth";
...
export default function ProfilePage() {
  ...
  const [adminUsers, setAdminUsers] = useState([]); // For tracking unique usernames in admin chart
  ...
  const fetchAdminStats = async () => {
    console.log("[Profile] Fetching admin stats for last 7 days...");
    try {
      const response = await authenticatedFetch(
        `${API_BASE.replace(/\/$/, "")}/admin/reconciliations/history?limit=1000`
      );
      ...
      if (response.ok) {
        ...
        // ⚡ Bolt Optimization: Replace O(N*M) nested loops with O(N+M) single-pass map
        const last7DaysSet = new Set(last7Days);
        const chartDataMap = new Map();
        const usernamesSet = new Set();

        // Initialize map with default objects per day
        ...
        data.forEach(recon => {
          ...
          // Fast path for ISO strings to avoid heavy Date parsing where possible
        ...
        const chartData = last7Days.map(date => {
          ...
          // Ensure all usernames have a value (0 if missing) for Recharts compatibility
          usernames.forEach(username => {
              if (!(username in dayData)) {
                  dayData[username] = 0;
              }
          });
          return dayData;
        ...
        setAdminChartData(chartData);
        setAdminUsers(usernames);
      }
    } catch (err) {
      console.error("[Profile] Failed to fetch admin stats:", err);
    }
  };
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <h1 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
                        {username}
                        {userIsAdmin && (
                          <span className="px-3 py-1.5 rounded-full text-sm font-bold bg-gradient-to-r from-amber-500 to-orange-500 text-white shadow-lg shadow-amber-500/30 flex items-center gap-1.5">
                            🔧 Administrator
                          </span>
                        )}
                      </h1>
            ...
            <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-2xl border border-white/20 dark:border-white/10 rounded-3xl p-8 shadow-xl">
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  <span className="p-2 bg-slate-100/50 dark:bg-slate-800/50 rounded-lg text-slate-600 dark:text-slate-400">
                    {userIsAdmin ? "📊" : "📈"}
                  </span>
                  {userIsAdmin ? "Daily Reconciliations (Last 7 Days)" : "Activity"}
                  {!userIsAdmin && <span className="text-sm font-normal text-slate-500 dark:text-slate-400 ml-2">(Last 30 Days)</span>}
                </h3>
              </div>

              {/* Conditional chart based on user role */}
              {/* User color palette */}
              {(() => {
                const USER_COLORS = [
                  "#f59e0b", // Amber
              ...
              {userIsAdmin ? (
                /* Admin: Show per-user reconciliation stats */
                adminChartData.length > 0 ? (
                  <div className="h-[350px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={adminChartData}>
                        ...
                        />
                        {/* Dynamic stacked bars for each user */}
                        {adminUsers.map((username, index) => {
                          ...
                          return (
                            <Bar
                              key={username}
                              dataKey={username}
                              stackId="reconciliations"
                              fill={USER_COLORS[index % USER_COLORS.length]}
                              radius={index === adminUsers.length - 1 ? [6, 6, 0, 0] : [0, 0, 0, 0]}
                            />
                          );
                        })}
                      </BarChart>
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-slate-500 dark:text-slate-400">
                    ...
                    <p className="text-sm">System-wide statistics will appear here</p>
                  </div>
                )
              ) : (
                /* Regular user: Show daily activity */
